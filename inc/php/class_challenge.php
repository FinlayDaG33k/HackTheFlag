<?php 
if(!empty($_POST['inputFlag']) && !empty($_SESSION['Username'])){
		...
		if ($sql_output->num_rows > 0){
			$check_solved = "SELECT * FROM logs WHERE `Username`='".mysqli_real_escape_string($conn,$_SESSION['Username'])."' AND `Challenge`='".$challenge_ID."' AND `Solved`='1'";
			...
			if($result->num_rows > 0){
				...
				$add_log = "INSERT INTO `logs` (`ID`, `Username`,`Challenge`, `Flag`, `Solved`, `Date`,`IP`) VALUES (NULL, '".mysqli_real_escape_string($conn,$_SESSION['Username'])."','".$challenge_ID."', '".mysqli_real_escape_string($conn,$_POST['inputFlag'])."', '".mysqli_real_escape_string($conn,$solved)."', '".date("Y-m-d H:i:s")."','".$_SERVER["HTTP_CF_CONNECTING_IP"]."');"; // SQL to get the flag
				...
				if($solved == 1){
					...
					$award_points = "UPDATE `users` SET `points` = points + ".$points." WHERE `Username` = '".mysqli_real_escape_string($conn,$_SESSION['Username'])."'";
